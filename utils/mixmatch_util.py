...
def mix_match(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    ...
    # step 1: Augmentation
    # 随机在输入上增加扰动
    X_cap = [(augmentation(x[0], aug_factor), x[1]) for x in X]   # 对标注数据进行加噪

    # batchsize翻K倍
    U_cap = U.repeat(K, 1, 1, 1, 1)  # [K*b, 1, D, H, W]
    # 一样加上一些随机扰动
    U_cap += torch.clamp(torch.randn_like(U_cap) * 0.1, -0.2, 0.2)  # 对无标注数据进行加噪声

    # step 2: label guessing
    with torch.no_grad():
        Y_u = eval_net(U_cap)  # (K, n_class, 112, 112, 80), (K, n_class, 112, 112, 80) 已经是one_hot的形式了
        # _, Y_u = eval_net(U_cap)  # (K, n_class, 112, 112, 80), (K, n_class, 112, 112, 80) 已经是one_hot的形式了
...
def mix_match_2(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    """
    参考论文 MixMatch: A Holistic Approach to Semi-Supervised Learning
    """
    # X is labeled data of size BATCH_SIZE, and U is unlabeled data
    ...
    # batchsize翻K倍
    U_cap = U.repeat(K, 1, 1, 1, 1)  # [K*b, 1, D, H, W]  对于无标注数据的简单翻倍扩增
    ...
    with torch.no_grad():
        # Y_u = eval_net(U_cap)  # (K, n_class, 112, 112, 80), (K, n_class, 112, 112, 80) 已经是one_hot的形式了
        Y_u, _= eval_net(U_cap)  # (K, n_class, 112, 112, 80), (K, n_class, 112, 112, 80) 已经是one_hot的形式了
        Y_u = F.softmax(Y_u, dim=1)
    ...
    for i in range(K):
        guessed += Y_u[i * U_b:(i + 1) * U_b]
    guessed /= K  # 将多次重复算个平均结果  原文的公式(6)
    guessed0 = torch.argmax(guessed,dim=1)
    guessed = guessed0.to('cpu').numpy()
    # guessed = guessed.argmax(1).numpy()
    pseudo_label = guessed
    # all_input = np.concatenate([x_in, U_cap], axis=0)
    # all_label = np.concatenate([x_lab, guessed], axis=0)
    guessed0.repeat(K,  1, 1, 1)
    U_cap = list(zip(U_cap, guessed0))  # 将伪标签和绕动数据合并
    ## Now we have X_cap ,list of (data, label) of length b, U_cap, list of (data, guessed_label) of length k*b
    # step 3: MixUp
    # original paper mathod
    x_mixup_mode, u_mixup_mode = mixup_mode[0], mixup_mode[1]
...
def mix_match_v2_soft_pth(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
...
def mix_match_v2_soft_pth_con(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
...
def mix_match_just_k1(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    ...
    # batchsize翻K倍
    U_cap = U.repeat(K, 1, 1, 1)  # [K*b, 1, D, H, W]
    ...
    idxs = np.random.permutation(range(U_b * K)) % X_b
    U_prime = [mix_up(U_cap[i], X[idxs[i]], alpha) for i in range(U_b * K)]

    return X_prime, U_prime, pseudo_label,X


def mix_match_just_k1_max_075(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor,mix_tau=0.75):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    idxs = np.random.permutation(range(U_b * K)) % X_b
    U_prime = [mix_up_075(U_cap[i], X[idxs[i]], alpha,mix_tau) for i in range(U_b * K)]

    return X_prime, U_prime, pseudo_label,X

def mix_match_just_k1_2D(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    with torch.no_grad():
        ...
        # print(Y_u.shape)
    guessed = torch.zeros(U.shape).repeat(1, K, 1, 1)  # empty label [b, 2, D, H, W]  这边是做成one-hot形式了
    # if GPU:
    guessed = guessed.cuda()
    if K !=1:
        for i in range(K):
...
def get_pseudo(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    # shape [U_b * K,2,D,H,W]  得到伪标签
    # pseudo_label = guessed
    return pseudo_label,U_cap


    # return X_prime, U_prime, pseudo_label,X_cap

def mix_match_just_k1_y_u(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    return X_prime, U_prime, pseudo_label,X_cap,Y_u


def mix_match_just_k1_2(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    return X_prime, U_prime, pseudo_label, X_cap,guessed_p


def mix_match_k1(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
...
def mix_match_v2_soft_pth_con_mix(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    # 随机在输入上增加扰动
    # aug_factor = torch.tensor(aug_factor)
    X_cap = X.repeat(K, 1, 1, 1, 1)
    X_cap += torch.clamp(torch.randn_like(X_cap) * 0.1, -0.2, 0.2)
    # X_cap = [(augmentation_torch(x[0], aug_factor), x[1]) for x in X]
    ...
    with torch.no_grad():
        # Y_u = eval_net(U_cap)  # (K, n_class, 112, 112, 80), (K, n_class, 112, 112, 80) 已经是one_hot的形式了
        Y_u1,_,_ = eval_net(U_cap[:2])
        Y_u2, _, _ = eval_net(U_cap[2:])
        Y_u = Y_u1 + Y_u2
        # Y_u = F.softmax(Y_u, dim=1)
    ...
    if x_mixup_mode == 'x':
        ...
        idxs = np.random.permutation(range(U_b * K))[:X_b]  # 保证取的范围在不标签的batch_size数的范围内, 取带标签的batch_size数个进行融合
        X_prime = [mix_up_only_img(X_cap[i], U_cap[idxs[i]], alpha) for i in range(X_b)]
    elif x_mixup_mode == '_':
        X_prime = X_cap
    elif x_mixup_mode == 's':
        X_prime = [mix_up(X_cap[i], X_cap[i], alpha) for i in range(X_b)]
    ...
    if u_mixup_mode == 'x':  # 保证取的范围在带标签的batch_size数的范围内, 取不带标签的batch_size数个进行融合
        idxs = np.random.permutation(range(U_b * K)) % X_b
        U_prime = [mix_up_only_img(U_cap[i], X_cap[idxs[i]], alpha) for i in range(U_b * K)]
    elif u_mixup_mode == 'u':  # 保证取的范围在不带标签的batch_size数的范围内, 取不带标签的batch_size数个进行融合
        idxs = np.random.permutation(range(U_b * K))
        U_prime = [mix_up(U_cap[i], U_cap[idxs[i]], alpha) for i in range(U_b * K)]  # 有問題???
    elif u_mixup_mode == '_':
...
def mix_match_3(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    """
    参考论文 MixMatch: A Holistic Approach to Semi-Supervised Learning
    """
    # X is labeled data of size BATCH_SIZE, and U is unlabeled data
...
def mix_match_pth_contrasive(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    with torch.no_grad():
        ...
        # if len(Y_u) > 1:
        #     Y_u = Y_u[0]
        # guessed = F.softmax(Y_u, dim=1)
        guessed = torch.argmax(Y_u[0], dim=1)
        # print(Y_u.shape)
    # guessed = torch.zeros(U.shape).repeat(1, K, 1, 1, 1)  # empty label [b, 2, D, H, W]  这边是做成one-hot形式了
    # if GPU:
    # guessed = guessed.cuda()
    ...
    return U_cap,U_prime, pseudo_label
    # return X_prime, U_prime, pseudo_label

def mix_match_pth_contrasive_augu_mix(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor,augus=None):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    # 随机在输入上增加扰动
    # aug_factor = torch.tensor(aug_factor)
    X_cap1 = augus.forward_image(X)
    X_cap2 = augus.forward_image(X)
    U_cap1 = augus.forward_image(U)
    U_cap2 = augus.forward_image(U)
    # X_cap = X
    ...
    with torch.no_grad():
        # Y_u = eval_net(U_cap)  # (K, n_class, 112, 112, 80), (K, n_class, 112, 112, 80) 已经是one_hot的形式了
        Y_u = eval_net(U_cap1) + eval_net(U_cap2)  # (K, n_class, 112, 112, 80), (K, n_class, 112, 112, 80) 已经是one_hot的形式了
        # if len(Y_u) > 1:
        #     Y_u = Y_u[0]
        # guessed = F.softmax(Y_u, dim=1)
        guessed = torch.argmax(Y_u[0], dim=1)
    ...
    # pseudo_labels = torch.cat([pseudo_label,pseudo_label],0)
    # U_cap = list(zip(U_cap, pseudo_labels))  # 将伪标签和绕动数据合并

    X_cap =torch.cat([X_cap1,X_cap2],0)
    # x_label = torch.cat([X[1],X[1]],0)
    # X_cap =  list(zip(X_caps, x_label))
    x_mixup_mode, u_mixup_mode = mixup_mode[0], mixup_mode[1]
    ...
    if u_mixup_mode == 'x':  # 保证取的范围在带标签的batch_size数的范围内, 取不带标签的batch_size数个进行融合
        idxs = np.random.permutation(range(U_b * K)) % X_b
        U_prime = [mix_up_torch(U_cap[i], X_cap[idxs[i]], alpha) for i in range(U_b * K)]
    elif u_mixup_mode == 'u':  # 保证取的范围在不带标签的batch_size数的范围内, 取不带标签的batch_size数个进行融合
        idxs = np.random.permutation(range(U_b * K))
        U_prime = [mix_up_torch(U_cap[i], U_cap[idxs[i]], alpha) for i in range(U_b * K)]  # 有問題???
    elif u_mixup_mode == '_':
        U_prime = U_cap
    else:
        raise ValueError('wrong mixup_mode')

    return X_cap,U_prime, pseudo_label
    # return X_prime, U_prime, pseudo_label


def mix_match_pth(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
...
def mix_match_pth_gan(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
    ...
    return U_cap,U_prime, pseudo_label,Y_u
    # return X_prime, U_prime, pseudo_label

def mix_match_pth_mix2(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
...
def mix_match_pth_gan_noise(X, U, eval_net, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)  # 应该是得到未标签的batch_size数才对
...
def mix_up_075(s1, s2, alpha,mix_tau=0.75):
    # print('??????', s1[0].shape, s1[1].shape, s2[0].shape, s2[1].shape)
    # s1, s2 are tuples(data, label)
    l = np.random.beta(alpha, alpha)  # 原文公式(8)
    l = max(l, 1 - l,mix_tau)  # 原文公式(9)
