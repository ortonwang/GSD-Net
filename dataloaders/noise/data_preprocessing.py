...
class DataPreprocessor:
    ...
    def __init__(
        ...
        self.image_dirs = {
            "train": self.root + "img/",
            "validation": self.root + "test_img/",
        }
        self.label_dirs = {
            "train": self.root + "mask/",
            "validation": self.root + "test_mask/",
        }
        self.class_label = "lesion" if self.dataname == "isic" else "lung"
        self.out_shape = (256, 256) if self.dataname == "isic" else (256, 256)
    ...
    def split_shenzhen_dataset(self) -> None:
        ...
        for phase in ["train", "validation"]:
            ...
            for name in tqdm(data_names[phase]):
                ...
                # print(arr)
                # arr = cv2.resize(arr, self.out_shape, interpolation=cv2.INTER_LINEAR)#*255
                # print(arr)
    ...
    def split_isic_dataset(self) -> None:
        ...
        for phase in ["train", "validation"]:
            ...
            for name in tqdm(data_names[phase]):
                ...
                src = _label_dirs[phase] + name + ".png"
